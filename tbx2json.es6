...
xml.on('endElement: termEntry', function (termEntry) {
	...
	const objStep1 = toJson.step1(termEntry);

	// Step 2: remove languages other than source and target
	const objStep2 = toJson.step2(objStep1, sourceLang, targetLang);
	let isObjStep2Empty = _.isEmpty(objStep2);
	...
	if (objStep3.length > 0) {
		let jsonStr = toJson.toJsonStr(objStep3, counter);
